...
pub fn compress(
    ...
    if (input.len > max_output and kind == .none) return error.SizeLimitExceeded;
    if (kind == .none) return allocator.dupe(u8, input);
    ...
    workspace.compressor.writer.writeAll(input) catch return outputFailure(&output);
    workspace.compressor.finish() catch return outputFailure(&output);
...
pub fn decompress(
    ...
    while (workspace.inflater.reader.peekGreedy(1)) |chunk| {
        const new_len = std.math.add(usize, output.items.len, chunk.len) catch
            return error.SizeLimitExceeded;
        try bounded_buffer.ensureCapacity(&output, allocator, new_len, max_output);
        output.appendSliceAssumeCapacity(chunk);
        workspace.inflater.reader.toss(chunk.len);
    } else |err| switch (err) {
        error.EndOfStream => {}, // NOOP: stream finished.
        error.ReadFailed => return error.MalformedCompressedData,
    }
    if (reject_trailing_bytes and source.bufferedLen() != 0) return error.TrailingData;
