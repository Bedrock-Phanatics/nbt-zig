...
pub const BoundedWriter = struct {
    ...
    pub fn init(
        allocator: std.mem.Allocator,
        max_len: usize,
    ) std.mem.Allocator.Error!BoundedWriter {
    ...
    fn ensureCapacity(
        ...
        self.writer.buffer = self.allocator.realloc(
            self.writer.buffer,
            capacity,
        ) catch return self.fail(.out_of_memory);
    }
    ...
    fn drain(
        ...
        const repeated_len = std.math.mul(
            usize,
            pattern.len,
            splat,
        ) catch return self.fail(.limit);
        ...
        for (data[0 .. data.len - 1]) |bytes| {
            @memcpy(
                writer.buffer[writer.end..][0..bytes.len],
                bytes,
            );
        ...
        for (0..splat) |_| {
            @memcpy(
                writer.buffer[writer.end..][0..pattern.len],
                pattern,
            );
