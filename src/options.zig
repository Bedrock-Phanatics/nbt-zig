...
pub const Options = struct {
    ...
    pub const bedrock: Options = .{ .encoding = .bedrock };
    pub const bedrock_network: Options = .{ .encoding = .bedrock_network };

    pub fn validate(self: Options) error{InvalidOptions}!void {
        if (self.max_depth == 0 or self.max_depth > 512 or self.max_input_bytes == 0 or self.max_decompressed_bytes == 0 or self.max_output_bytes == 0 or self.max_total_decoded_bytes == 0)
